...
- 有效 LCD 分辨率为 159x96，通过软件缩放到 240x145。LCD 与触摸按键之间
  的齿轮按钮可选择最近邻、双线性或原始分辨率显示，默认使用双线性；齿轮
  左侧的图标按钮用于切换游戏控制面板与软键盘。软键盘模式会显示 `FN/ABC`
  分页按钮，分别进入原机功能键页和字母数字键页。中间栏不显示缩放名称。
...
- 设置：点击齿轮按钮；在设置面板中使用上/下键和 Enter，或直接点击对应
  项目。`COLOR` 在关闭、绿色、蓝色、黄色之间循环，`GHOST` 控制 LCD
  残影。点击面板的 X 按钮可关闭设置。
- 重新启动：选择 `RESTART GAME`，应用会先写回当前 `.sav`，完整释放当前
  模拟器会话，然后使用同一个 `.gam` 路径重新初始化。
- 更换游戏：选择 `CHANGE GAME`，应用会在当前会话保存并释放后重新打开
  `A:\gam4980\` 文件选择器。取消选择时会重新启动原游戏。

选择的缩放算法、游戏控制面板/软键盘模式、LCD 颜色主题和残影开关保存在：
...
原始灰白配色和关闭残影。旧版本创建的 8 字节配置文件可直接继续使用，
其预留字节会使新增选项保持关闭。
